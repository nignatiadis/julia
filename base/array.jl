...
dimension. `Vector{T}(m)` and `Matrix{T}(m, n)` are stricly equivalent aliases to create one-
...
dimensionality of the result. `Vector{T}(itr)` and `Matrix{T}(itr)` are stricly
equivalent aliases to create one- and two-dimensional arrays.
